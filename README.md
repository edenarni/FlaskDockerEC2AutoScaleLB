...
1. **[Running Locally with Docker Compose](#quick-start-guide-running-dockerized-application-with-docker-compose-locally)**: You can run the application on your local machine using Docker Compose, which ensures consistency across different environments.

2. **[Deploying to an EC2 Instance](#quick-deployment-guide-deploying-your-dockerized-application-to-an-ec2-instance)**: You can deploy the application to an AWS EC2 instance for scalable and high-availability deployment, leveraging AWS services like Auto Scaling and Load Balancing.


Follow the respective guides below to get started with your preferred deployment option.

<br><br>


## Quick Start Guide: Running Dockerized Application with Docker Compose (locally)

### Prerequisites

Ensure you have the following software installed on your computer:

1. **Git**: For cloning the repository from GitHub.
2. **Docker**: To run the Docker containers.
3. **Docker Compose**: To orchestrate the multi-container Docker application.
4. **Python and pip**: To install the Python dependencies (although the dependencies will be managed inside the Docker container, having Python and pip can be useful for initial setup or troubleshooting).
...
2. **Navigate to the Repository Directory**: Move into the cloned repository's directory. 
```
...
5. **Run Docker Compose**: Start the application defined in the `docker-compose.yml` file.
```
...
- In the **IAM instance profile** section:  
  ...
  <img width="700" alt="image" src="https://github.com/edenarni/FlaskDockerEC2AutoScaleLB/assets/123691333/38a49ed9-895a-48af-83a0-59977adee69e">  
...
- Finally, this is the template instance we created: <br>
  <img width="700" alt="image" src="https://github.com/edenarni/FlaskDockerEC2AutoScaleLB/assets/123691333/467802c4-fc06-4894-9f6b-083f4c05a24e">  
...
By following this guide, you should be able to set up and deploy a Flask application using Docker, with PostgreSQL for the database, hosted on an AWS EC2 instance with auto-scaling and load balancing. The application securely retrieves images from a private S3 bucket, demonstrating integration with various AWS services.

## Troubleshooting

### Common Issues
- **Docker Compose fails to start:** Ensure that Docker and Docker-Compose is properly installed and running on your machine. Verify your `docker-compose.yml` file for syntax errors.
- **EC2 instance not launching:** Double-check your EC2 launch template configuration, especially the user data script and security group settings.
- **Access denied to S3 bucket:** Ensure your AWS credentials and bucket policies are correctly set. The S3 bucket must be private, and your IAM user needs the necessary permissions to access the bucket.
